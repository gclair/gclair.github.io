...
return Result;};ByteArray.prototype.writeByte=function(value){this._Bytes[this._Position++]=(value&0xFF);if(this._Position>this._Length){this._Length++;}};ByteArray.prototype.writeBytes=function(bytes,offset,length){if(!offset){offset=0;}
if(!length){length=0;}
...
bytes.position=BytesPosition;};ByteArray.prototype.writeShort=function(value){this.writeByte((value&0xFF00)>>8);this.writeByte(value&0x00FF);};ByteArray.prototype.writeString=function(text){var Textlength=text.length;for(var i=0;i<Textlength;i++){this.writeByte(text.charCodeAt(i)&0xFF);}};return ByteArray;}());var ClipboardHelper=(function(){function ClipboardHelper(){}
...
break;case'H':case'f':y=Math.max(1,this.GetNextParam(1));x=Math.max(1,this.GetNextParam(1));this._Crt.GotoXY(x,y);break;case'h':if(this._AnsiParams.length<1){this._AnsiParams.push('0');}
switch(this._AnsiParams[0]){case'=255':console.log('Unhandled ESC sequence: Enable DoorWay Mode');break;case'?6':console.log('Unhandled ESC sequence: Enable origin mode');break;case'?7':console.log('Unhandled ESC sequence: Enable auto wrap');break;case'?25':this._Crt.ShowCursor();break;case'?31':console.log('Unhandled ESC sequence: Enable alt character set');break;case'?32':console.log('Unhandled ESC sequence: Bright Intensity Enable');break;case'?33':console.log('Unhandled ESC sequence: Blink to Bright Intensity Background');break;default:console.log('Unknown ESC sequence: PB('+this._AnsiParams.toString()+') IB('+this._AnsiIntermediates.toString()+') FB('+finalByte+')');break;}
break;case'J':switch(this.GetNextParam(0)){case 0:this._Crt.ClrEos();break;case 1:this._Crt.ClrBos();break;case 2:this._Crt.ClrScr();break;case 3:this._Crt.ClrScr();break;}
...
break;case'S':y=Math.max(1,this.GetNextParam(1));this._Crt.ScrollUpScreen(y);break;case's':if(this._AnsiIntermediates.length===0){this._AnsiXY=new Point(this._Crt.WhereX(),this._Crt.WhereY());}
else{console.log('Unhandled ESC sequence: Save Mode Setting');}
break;case'T':y=Math.max(1,this.GetNextParam(1));this._Crt.ScrollDownWindow(y);break;case't':if(this._AnsiParams.length===4){switch(this.GetNextParam(1)){case 0:this._Crt.TextBackground24(this.GetNextParam(0),this.GetNextParam(0),this.GetNextParam(0));break;case 1:this._Crt.TextColor24(this.GetNextParam(0),this.GetNextParam(0),this.GetNextParam(0));break;default:console.log('Unknown ESC sequence: PB('+this._AnsiParams.toString()+') IB('+this._AnsiIntermediates.toString()+') FB('+finalByte+')');break;}}
else{console.log('Unknown ESC sequence: PB('+this._AnsiParams.toString()+') IB('+this._AnsiIntermediates.toString()+') FB('+finalByte+')');}
...
break;case'X':x=Math.max(1,this.GetNextParam(1));this._Crt.DelChar(x);break;case'Z':console.log('Unhandled ESC sequnce: Cursor Backward Tabulation');break;default:console.log('Unknown ESC sequence: PB('+this._AnsiParams.toString()+') IB('+this._AnsiIntermediates.toString()+') FB('+finalByte+')');break;}};Ansi.prototype.CursorPosition=function(x,y){if(typeof x==='undefined'){x=this._Crt.WhereXA();}
...
else{return parseInt(Result,10);}};Ansi.prototype.Write=function(text){if(this._Crt.Atari||this._Crt.C64){this._Crt.Write(text);}
else{var Buffer='';for(var i=0;i<text.length;i++){if(text.charAt(i)==='\x1B'){this._AnsiParserState=AnsiParserState.Escape;}
...
else if((text.charAt(i)>='0')&&(text.charAt(i)<='?')){this._AnsiBuffer+=text.charAt(i);this._AnsiParserState=AnsiParserState.ParameterByte;}
else if((text.charAt(i)>=' ')&&(text.charAt(i)<='/')){this._AnsiBuffer+=text.charAt(i);this._AnsiParserState=AnsiParserState.IntermediateByte;}
...
Object.defineProperty(Crt.prototype,"AllowDynamicFontResize",{get:function(){return this._AllowDynamicFontResize;},set:function(value){this._AllowDynamicFontResize=value;},enumerable:true,configurable:true});Object.defineProperty(Crt.prototype,"Atari",{get:function(){return this._Atari;},set:function(value){this._Atari=value;},enumerable:true,configurable:true});Object.defineProperty(Crt.prototype,"BareLFtoCRLF",{get:function(){return this._BareLFtoCRLF;},set:function(value){this._BareLFtoCRLF=value;},enumerable:true,configurable:true});Crt.prototype.Beep=function(){};Object.defineProperty(Crt.prototype,"C64",{get:function(){return this._C64;},set:function(value){this._C64=value;},enumerable:true,configurable:true});Object.defineProperty(Crt.prototype,"Canvas",{get:function(){return this._Canvas;},enumerable:true,configurable:true});Crt.prototype.ClrBol=function(){this.FastWrite(StringUtils.NewString(' ',this.WhereX()),this.WindMinX+1,this.WhereYA(),this._CharInfo);};Crt.prototype.ClrBos=function(){this.ScrollUpWindow(this.WhereY()-1);this.ScrollDownWindow(this.WhereY()-1);this.ClrBol();};Crt.prototype.ClrEol=function(){this.FastWrite(StringUtils.NewString(' ',(this.WindMaxX+1)-this.WhereX()+1),this.WhereXA(),this.WhereYA(),this._CharInfo);};Crt.prototype.ClrEos=function(){this.ScrollDownWindow(this.WindRows-this.WhereY());this.ScrollUpWindow(this.WindRows-this.WhereY());this.ClrEol();};Crt.prototype.ClrLine=function(){this.FastWrite(StringUtils.NewString(' ',this.WindCols),this.WindMinX+1,this.WhereYA(),this._CharInfo);};Crt.prototype.ClrScr=function(){this.ScrollUpWindow(this.WindRows);this.GotoXY(1,1);};Crt.prototype.Conceal=function(){this.TextColor((this.TextAttr&0xF0)>>4);};Crt.prototype.DelChar=function(count){if(typeof count==='undefined'){count=1;}
...
this._InScrollback=false;};Crt.prototype.FastWrite=function(text,x,y,charInfo,updateBuffer){if(typeof updateBuffer==='undefined'){updateBuffer=true;}
...
this._MouseMovePoint=new Point(this._MouseDownPoint.x,this._MouseDownPoint.y);};Crt.prototype.OnMouseMove=function(me){if(typeof this._MouseDownPoint==='undefined'){return;}
...
BScrollUpdateBuffer.Stop();};Crt.prototype.ScrollUpScreen=function(count){this.ScrollUpCustom(1,1,this._ScreenSize.x,this._ScreenSize.y,count,this._CharInfo);};Crt.prototype.ScrollUpWindow=function(count){this.ScrollUpCustom(this.WindMinX+1,this.WindMinY+1,this.WindMaxX+1,this.WindMaxY+1,count,this._CharInfo);};Crt.prototype.SetBlink=function(value){this._CharInfo.Blink=value;};Crt.prototype.SetBlinkRate=function(milliSeconds){this._Cursor.BlinkRate=milliSeconds;};Crt.prototype.SetFont=function(font){if(this._UseModernScrollback){if(this._Container.parentElement===null){return this._Font.Load(font,Math.floor(this._Container.clientWidth/this._ScreenSize.x),Math.floor(window.innerHeight/this._ScreenSize.y));}
else{return this._Font.Load(font,Math.floor(this._Container.parentElement.clientWidth/this._ScreenSize.x),Math.floor(window.innerHeight/this._ScreenSize.y));}}
else{return this._Font.Load(font,Math.floor(this._Container.clientWidth/this._ScreenSize.x),Math.floor(window.innerHeight/this._ScreenSize.y));}};Crt.prototype.SetScreenSize=function(columns,rows){if(this._InScrollback){return;}
...
else{this.WriteASCII(text);}};Crt.prototype.WriteASCII=function(text){if(typeof text==='undefined'){text='';}
var X=this.WhereX();var Y=this.WhereY();var Buf='';for(var i=0;i<text.length;i++){var DoGoto=false;if(text.charCodeAt(i)===0x00){i+=0;}
else if(text.charCodeAt(i)===0x07){this.Beep();}
...
else if((text.charCodeAt(i)===0xFD)&&(!this._ATASCIIEscaped)){this.Beep();}
...
return this._CharMap[CharMapKey];};Object.defineProperty(CrtFont.prototype,"Height",{get:function(){return this._Size.y;},enumerable:true,configurable:true});CrtFont.prototype.Load=function(font,maxWidth,maxHeight){var _this=this;var BestFit;if(font.indexOf('_')>=0){if(CrtFonts.HasFont(font)){var NameSize=font.split('_');var WidthHeight=NameSize[1].split('x');BestFit=new Point(parseInt(WidthHeight[0],10),parseInt(WidthHeight[1],10));font=NameSize[0];}}
...
else{Protocols=['base64','plain'];}}
var WsOrWss=forceWss?'wss':'ws';if(proxyHostname===''){this._WebSocket=new WebSocket(WsOrWss+'://'+hostname+':'+port+urlPath,Protocols);}
...
Data.position=0;this.NegotiateInbound(Data);this._InputBuffer.position=OldPosition;this.ondata.trigger();};WebSocketConnection.prototype.readBytes=function(bytes,offset,length){return this._InputBuffer.readBytes(bytes,offset,length);};WebSocketConnection.prototype.readString=function(length){return this._InputBuffer.readString(length);};WebSocketConnection.prototype.readUnsignedByte=function(){return this._InputBuffer.readUnsignedByte();};WebSocketConnection.prototype.readUnsignedShort=function(){return this._InputBuffer.readUnsignedShort();};WebSocketConnection.prototype.Send=function(data){var DebugLine="";if(UseCordovaSocket){this._CordovaSocket.write(new Uint8Array(data));}
else{var i=0;var ToSendString='';if(this._Protocol==='binary'){this._WebSocket.send(new Uint8Array(data).buffer);}
else if(this._Protocol==='base64'){for(i=0;i<data.length;i++){var B=data[i];if(B>=32&&B<=126){DebugLine+=String.fromCharCode(B);}
else{DebugLine+='~'+B.toString(10);}
ToSendString+=String.fromCharCode(data[i]);}
this._WebSocket.send(btoa(ToSendString));}
else{for(i=0;i<data.length;i++){var B=data[i];if(B>=32&&B<=126){DebugLine+=String.fromCharCode(B);}
else{DebugLine+='~'+B.toString(10);}
ToSendString+=String.fromCharCode(data[i]);}
this._WebSocket.send(ToSendString);}}
if(this._LogIO){console.log('SEND: '+DebugLine);}};Object.defineProperty(WebSocketConnection.prototype,"SendLocation",{set:function(value){this._SendLocation=value;},enumerable:true,configurable:true});WebSocketConnection.prototype.writeByte=function(value){this._OutputBuffer.writeByte(value);};WebSocketConnection.prototype.writeBytes=function(bytes,offset,length){this._OutputBuffer.writeBytes(bytes,offset,length);};WebSocketConnection.prototype.writeShort=function(value){this._OutputBuffer.writeShort(value);};WebSocketConnection.prototype.writeString=function(text){this._OutputBuffer.writeString(text);this.flush();};return WebSocketConnection;}());var __extends=(this&&this.__extends)||(function(){var extendStatics=function(d,b){extendStatics=Object.setPrototypeOf||({__proto__:[]}instanceof Array&&function(d,b){d.__proto__=b;})||function(d,b){for(var p in b)if(Object.prototype.hasOwnProperty.call(b,p))d[p]=b[p];};return extendStatics(d,b);};return function(d,b){extendStatics(d,b);function __(){this.constructor=d;}
...
else{this.SendWont(TelnetOption.SendLocation);}};TelnetConnection.prototype.HandleTerminalType=function(){this.SendWill(TelnetOption.TerminalType);this.SendSubnegotiate(TelnetOption.TerminalType);var TerminalType=this._TerminalTypes[this._TerminalTypeIndex];var ToSendBytes=[];ToSendBytes.push(0);for(var i=0;i<TerminalType.length;i++){ToSendBytes.push(TerminalType.charCodeAt(i));}
...
else{this._TerminalTypeIndex=0;}};TelnetConnection.prototype.HandleTerminalLocationNumber=function(){var _this=this;if(this._SendLocation){var xhr=new XMLHttpRequest();xhr.open('get','https://text.ipv4.wtfismyip.com/',true);xhr.onload=function(){var status=xhr.status;if(status===200){_this.SendWill(TelnetOption.TerminalLocationNumber);_this.SendSubnegotiate(TelnetOption.TerminalLocationNumber);var InternetHostNumber=StringUtils.IPtoInteger(xhr.responseText);var TerminalNumber=0xFFFFFFFF;var SixtyFourBits=[];SixtyFourBits.push(0);SixtyFourBits.push((InternetHostNumber&0xFF000000)>>24);SixtyFourBits.push((InternetHostNumber&0x00FF0000)>>16);SixtyFourBits.push((InternetHostNumber&0x0000FF00)>>8);SixtyFourBits.push((InternetHostNumber&0x000000FF)>>0);SixtyFourBits.push((TerminalNumber&0xFF000000)>>24);SixtyFourBits.push((TerminalNumber&0x00FF0000)>>16);SixtyFourBits.push((TerminalNumber&0x0000FF00)>>8);SixtyFourBits.push((TerminalNumber&0x000000FF)>>0);var ToSendBytes=[];for(var i=0;i<SixtyFourBits.length;i++){ToSendBytes.push(SixtyFourBits[i]);if(SixtyFourBits[i]===TelnetCommand.IAC){ToSendBytes.push(TelnetCommand.IAC);}}
_this.Send(ToSendBytes);_this.SendSubnegotiateEnd();}
...
else{this.SendWont(TelnetOption.Echo);}}},enumerable:true,configurable:true});TelnetConnection.prototype.NegotiateInbound=function(data){var DebugLine="";while(data.bytesAvailable){var B=data.readUnsignedByte();if(B>=32&&B<=126){DebugLine+=String.fromCharCode(B);}
else{DebugLine+='~'+B.toString(10);}
if(this._NegotiationState===TelnetNegotiationState.Data){if(B===TelnetCommand.IAC){this._NegotiationState=TelnetNegotiationState.IAC;}
else{this._InputBuffer.writeByte(B);}}
...
this._NegotiationState=TelnetNegotiationState.Data;}
else{this._NegotiationState=TelnetNegotiationState.Data;}}
if(this._LogIO){if(DebugLine.length>0){console.log('IN: '+DebugLine);}}};TelnetConnection.prototype.OnSocketOpen=function(){_super.prototype.OnSocketOpen.call(this);if(this._LocalEcho){this.SendWill(TelnetOption.Echo);}
else{this.SendWont(TelnetOption.Echo);}
if(this._SendLocation){this.SendWill(TelnetOption.SendLocation);this.SendWill(TelnetOption.TerminalLocationNumber);}};TelnetConnection.prototype.SendDo=function(option){if(this._NegotiatedOptions[option]!==TelnetCommand.Do){this._NegotiatedOptions[option]=TelnetCommand.Do;var ToSendBytes=[];ToSendBytes.push(TelnetCommand.IAC);ToSendBytes.push(TelnetCommand.Do);ToSendBytes.push(option);this.Send(ToSendBytes);if(this._LogIO){console.log('DO '+option.toString(10));}}
...
else if((B>=33)&&(B<=126)){this._Crt.PushKeyPress(B,0,false,false,false);}}};fTelnetClient.prototype.Connect=function(){var _this=this;if(typeof this._MenuButtons!=='undefined'){this._MenuButtons.style.display='none';}
...
else if(document.webkitExitFullscreen){document.webkitExitFullscreen();}}};fTelnetClient.prototype.OnAnsiESC0c=function(){if(typeof this._Connection==='undefined'){return;}
if(!this._Connection.connected){return;}
...
if(!this._Connection.connected){return;}
this._Connection.writeString(this._Ansi.CursorPosition());};fTelnetClient.prototype.OnAnsiESC255n=function(){if(typeof this._Connection==='undefined'){return;}
if(!this._Connection.connected){return;}
...
if(this._Connection.bytesAvailable>0){clearTimeout(this._DataTimer);this._DataTimer=setTimeout(function(){_this.OnConnectionData();},50);}}}
...
else{this._Connection.writeString(KPE.keyString);}}}}}}};fTelnetClient.prototype.OnCrtScreenSizeChanged=function(){var NewWidth;var NewHeight;if(this._Options.Emulation==='RIP'){NewWidth=640;}
else{if(this._UseModernScrollback){NewWidth=this._Crt.ScreenCols*this._Crt.Font.Width+GetScrollbarWidth.Width;NewHeight=this._Crt.ScreenRows*this._Crt.Font.Height;this._ClientContainer.style.width=NewWidth+'px';this._ClientContainer.style.height=NewHeight+'px';this._ClientContainer.scrollTop=this._ClientContainer.scrollHeight;}
else{NewWidth=this._Crt.ScreenCols*this._Crt.Font.Width;}}
...
if((document.getElementById('fTelnetScript')!==null)&&(document.getElementById('fTelnetKeyboardCss')!==null)){var KeyboardSizes=[960,800,720,640,560,480,360,320];for(var i=0;i<KeyboardSizes.length;i++){if(((NewWidth>=KeyboardSizes[i])&&(KeyboardSizes[i]<=screen.width))||(i===(KeyboardSizes.length-1))){document.getElementById('fTelnetKeyboardCss').href=StringUtils.GetUrl('keyboard/keyboard-'+KeyboardSizes[i].toString(10)+'.min.css');break;}}}};fTelnetClient.prototype.OnDownloadComplete=function(){var _this=this;this._Timer=setInterval(function(){_this.OnTimer();},250);};fTelnetClient.prototype.OnMenuButtonClick=function(){this._MenuButtons.style.display=(this._MenuButtons.style.display==='none')?'block':'none';this._MenuButtons.style.left=Offset.getOffset(this._MenuButton).x+this._MenuButton.clientWidth+'px';this._MenuButtons.style.top=Offset.getOffset(this._MenuButton).y-this._MenuButtons.clientHeight+'px';};fTelnetClient.prototype.OnTimer=function(){if((typeof this._Connection!=='undefined')&&(this._Connection.connected)){if(document.hasFocus()&&!this._HasFocus){this._HasFocus=true;this._FocusWarningBar.style.display='none';}
